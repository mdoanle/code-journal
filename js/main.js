  ...
  if (data.editing === null) {
    ...
    for (var i = 0; i < data.entries.length; i++) {
      ...
      if (parseInt(data.entries[i].entryID) === editedID) {
        ...
        existingEntryData.notesInput = $formEntry.elements.notes.value;
...
function deleteEntry(event) {
  var $editedEntry = document.querySelector(`li[data-entry-id="${data.editing.entryID}"]`);
  data.entries = data.entries.filter(entry => entry.entryID !== data.editing.entryID);
  if ($editedEntry != null) {
    $editedEntry.remove();
  }
  viewSwap('entries');
